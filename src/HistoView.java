...
public class HistoView extends JPanel {
	...
	class HistoScreen extends JComponent {
		...
		public void paintComponent(Graphics g) {
			int[] sortedHistogram = histogram.clone();
			int lines;
			...
			Arrays.sort(sortedHistogram); // sort histogram values into ascending order
			double maxValue = sortedHistogram[255]; // highest peak in histogram
			
			for(int i = 0; i < histogram.length; i++){
				lines = (int) ((histogram[i] / maxValue)*height); // scale lines depending on image height
				g.drawLine(i, height, i, height-lines);
			}
		}
